...
def row_to_singer_record(catalog_entry, version, row, columns, time_extracted, config):
    ...
    for idx, elem in enumerate(row):
        ...
        if isinstance(elem, datetime.datetime):
            row_to_persist += (elem.isoformat(timespec='milliseconds'),)
            ...
            if use_date_data_type_format:
                ...
                row_to_persist += (elem.isoformat(timespec='milliseconds') + "T00:00:00",)
                # row_to_persist += (elem.isoformat() + "T00:00:00+00:00",)

        elif isinstance(elem, datetime.timedelta):
            epoch = datetime.datetime.utcfromtimestamp(0)
            timedelta_from_epoch = epoch + elem
            row_to_persist += (timedelta_from_epoch.isoformat(timespec='milliseconds'),)
